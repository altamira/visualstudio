...
/****** Object:  Stored Procedure dbo.SPCP_DESTINACAOPEND_CONSULTA    Script Date: 23/10/2010 13:58:20 ******/


/****** Object:  Stored Procedure dbo.SPCP_DESTINACAOPEND_CONSULTA    Script Date: 25/08/1999 20:11:54 ******/
CREATE PROCEDURE SPCP_DESTINACAOPEND_CONSULTA
   ...
   SELECT @TotalFolha = IsNull(@TotalFolha, 0) + IsNull(SUM(cpnd_ValorTotal), 0)
      FROM CP_NotaFiscalDetalhe
           WHERE cpnd_DataVencimento BETWEEN @DataInicial AND @DataFinal
             ...
             AND cpnd_Grupo = 1
   SELECT @TotalFolha = IsNull(@TotalFolha, 0) + IsNull(SUM(cpdd_ValorTotal), 0)
      FROM CP_DespesaImpostoDetalhe
   ...
   SELECT @TotalPrestacao = IsNull(@TotalPrestacao, 0) + IsNull(SUM(cpnd_ValorTotal), 0)
      FROM CP_NotaFiscalDetalhe
           WHERE cpnd_DataVencimento BETWEEN @DataInicial AND @DataFinal
             ...
             AND cpnd_DataPagamento IS NULL
             AND cpnd_Grupo = 2
   SELECT @TotalPrestacao = IsNull(@TotalPrestacao, 0) + IsNull(SUM(cpdd_ValorTotal), 0)
      FROM CP_DespesaImpostoDetalhe
           WHERE cpdd_DataProrrogacao BETWEEN @DataInicial AND @DataFinal
             AND cpdd_DataPagamento IS NULL
             AND cpdd_Grupo = 2
   ...
   SELECT @TotalTransporte = IsNull(@TotalTransporte, 0) + IsNull(SUM(cpnd_ValorTotal), 0)
      FROM CP_NotaFiscalDetalhe
           WHERE cpnd_DataVencimento BETWEEN @DataInicial AND @DataFinal
             ...
             AND cpnd_DataPagamento IS NULL
             AND cpnd_Grupo = 3
   SELECT @TotalTransporte = IsNull(@TotalTransporte, 0) + IsNull(SUM(cpdd_ValorTotal), 0)
      FROM CP_DespesaImpostoDetalhe
           WHERE cpdd_DataProrrogacao BETWEEN @DataInicial AND @DataFinal
             AND cpdd_DataPagamento IS NULL 
             AND cpdd_Grupo = 3
   ...
   SELECT @TotalDespesa = IsNull(@TotalDespesa, 0) + IsNull(SUM(cpnd_ValorTotal), 0)
      FROM CP_NotaFiscalDetalhe
           WHERE cpnd_DataVencimento BETWEEN @DataInicial AND @DataFinal
             ...
             AND cpnd_DataPagamento IS NULL
             AND cpnd_Grupo = 4
   SELECT @TotalDespesa = IsNull(@TotalDespesa, 0) + IsNull(SUM(cpdd_ValorTotal), 0)
      FROM CP_DespesaImpostoDetalhe
           WHERE cpdd_DataProrrogacao BETWEEN @DataInicial AND @DataFinal
             AND cpdd_DataPagamento IS NULL
             AND cpdd_Grupo = 4
   ...
   SELECT @TotalFornecedor = IsNull(@TotalFornecedor, 0) + IsNull(SUM(cpnd_ValorTotal), 0)
      FROM CP_NotaFiscalDetalhe
           WHERE cpnd_DataVencimento BETWEEN @DataInicial AND @DataFinal
             ...
             AND cpnd_DataPagamento IS NULL
             AND cpnd_Grupo = 5
   SELECT @TotalFornecedor = IsNull(@TotalFornecedor, 0) + IsNull(SUM(cpdd_ValorTotal), 0)
      FROM CP_DespesaImpostoDetalhe
           WHERE cpdd_DataProrrogacao BETWEEN @DataInicial AND @DataFinal
             AND cpdd_DataPagamento IS NULL
             AND cpdd_Grupo = 5
   ...
   SELECT @TotalImobilizado = IsNull(@TotalImobilizado, 0) + IsNull(SUM(cpnd_ValorTotal), 0)
      FROM CP_NotaFiscalDetalhe
           WHERE cpnd_DataVencimento BETWEEN @DataInicial AND @DataFinal
             ...
             AND cpnd_DataPagamento IS NULL
             AND cpnd_Grupo = 6
   SELECT @TotalImobilizado = IsNull(@TotalImobilizado, 0) + IsNull(SUM(cpdd_ValorTotal), 0)
      FROM CP_DespesaImpostoDetalhe
           WHERE cpdd_DataProrrogacao BETWEEN @DataInicial AND @DataFinal
             AND cpdd_DataPagamento IS NULL
             AND cpdd_Grupo = 6
   ...
   SELECT @TotalManutencao = IsNull(@TotalManutencao, 0) + IsNull(SUM(cpnd_ValorTotal), 0)
      FROM CP_NotaFiscalDetalhe
           WHERE cpnd_DataVencimento BETWEEN @DataInicial AND @DataFinal
             ...
             AND cpnd_DataPagamento IS NULL
             AND cpnd_Grupo = 7
   SELECT @TotalManutencao = IsNull(@TotalManutencao, 0) + IsNull(SUM(cpdd_ValorTotal), 0)
      FROM CP_DespesaImpostoDetalhe
           WHERE cpdd_DataProrrogacao BETWEEN @DataInicial AND @DataFinal
             AND cpdd_DataPagamento IS NULL
             AND cpdd_Grupo = 7
   ...
   SELECT @TotalImposto = IsNull(@TotalImposto, 0) + IsNull(SUM(cpnd_ValorTotal), 0)
      FROM CP_NotaFiscalDetalhe
           WHERE cpnd_DataVencimento BETWEEN @DataInicial AND @DataFinal
             ...
             AND cpnd_DataPagamento IS NULL
             AND cpnd_Grupo = 8
   SELECT @TotalImposto = IsNull(@TotalImposto, 0) + IsNull(SUM(cpdd_ValorTotal), 0)
      FROM CP_DespesaImpostoDetalhe
           WHERE cpdd_DataProrrogacao BETWEEN @DataInicial AND @DataFinal
             AND cpdd_DataPagamento IS NULL
             AND cpdd_Grupo = 8
