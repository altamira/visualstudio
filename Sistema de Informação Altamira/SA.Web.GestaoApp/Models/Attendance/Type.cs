...
namespace GestaoApp.Models.Attendance
{
    public class Type : Base
    {
        ...
        private static ObservableCollection<Type> collection;
        ...
        public static ObservableCollection<Type> Collection
        {
            get
            {
                if (collection == null)
            ...
            try
            {
                HttpRequestHelper httpRequest =
                    new HttpRequestHelper(
                        new Uri(string.Format("http://{0}:{1}/Attendance.Type.List", Application.Current.Host.Source.Host, Application.Current.Host.Source.Port)),
        ...
        public static void OnReceiveCompleted(HttpResponseCompleteEventArgs e)
        {
            if (System.Windows.Deployment.Current.Dispatcher.CheckAccess())
            {
                if (e.Error == null)
                {
                    ...
                    {
                        collection = new ObservableCollection<Type>
                                    (from type in xParse.Descendants("Type")
                                     select new Type() { ToXML = type });
