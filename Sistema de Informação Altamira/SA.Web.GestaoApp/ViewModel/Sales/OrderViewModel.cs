...
{
    public class OrderViewModel : SearchViewModel<GestaoApp.Models.Sales.Order>
    {
        #region Attributes
        ...
        #region Properties
        public override Order SelectedItem
        {
            get
            {
        ...
        #region Constructor
        public OrderViewModel(string parameter)
        {
            CanAdd = true;
            CanSearch = true;
        ...
        {
            if (SelectedItem.Bid.Documents.Count == 0)
            {
                CustomMessage msg = new CustomMessage("Nenhum arquivo encontrado !", CustomMessage.MessageType.Info);
                msg.Show();
                return;
            }
            else if (SelectedItem.Bid.Documents.Count == 1)
                OpenDocument(SelectedItem.Bid.Documents.First(), new EventArgs());
            else
            {
                DocumentViewModel viewmodel = new DocumentViewModel();
                viewmodel.SearchResultList = SelectedItem.Bid.Documents;
        ...
        private void OpenDocument(object sender, EventArgs e)
        {
            ...
            if (GestaoApp.ViewModel.ViewModelBase.Session.Rules != "*" &&
                !GestaoApp.ViewModel.ViewModelBase.Session.Rules.Contains("Bid Order"))
            {
            ...
            view.htmlHost.SourceUri = new Uri(string.Format("http://{0}:{1}/Sales.Bid.Document?{2}&{3}&{4}&{5}", 
                ...
                int.Parse(SelectedItem.Bid.Number).ToString(),
                HttpUtility.UrlEncode(SelectedItem.Bid.Revision.Trim()),
        ...
        {
            if (SelectedItem.Bid.Projects.Count == 0)
            {
                CustomMessage msg = new CustomMessage("Nenhum arquivo encontrado !", CustomMessage.MessageType.Info);
                msg.Show();
                return;
            }
            else if (SelectedItem.Bid.Projects.Count == 1)
                OpenProject(SelectedItem.Bid.Projects.First(), new EventArgs());
            else
            {
                DocumentViewModel viewmodel = new DocumentViewModel();
                viewmodel.SearchResultList = SelectedItem.Bid.Projects;
        ...
        OrderFormView childWindow;
        Order PendingChanges;

        public override void Add(object parameter)
        {
            OrderViewModel viewmodel = new OrderViewModel();
            viewmodel.SelectedItem = new Order();
            childWindow = new OrderFormView();
        ...
        {
            if (sender is OrderFormView)
                if ((sender as OrderFormView).DataContext is OrderViewModel)
                {
                    PendingChanges = ((sender as OrderFormView).DataContext as OrderViewModel).selectedItem as Order;
        ...
        public override void Search(object parameter)
        {
            try
            {
                HttpRequestHelper httpRequest =
                    new HttpRequestHelper(
                        new Uri(string.Format("http://{0}:{1}/Sales.Order.Search?{2}", Application.Current.Host.Source.Host, Application.Current.Host.Source.Port, Session.Guid)),
                        "POST",
                        new XDocument
                        (
        ...
        public void OnSearchCompleted(HttpResponseCompleteEventArgs e)
        {
            if (System.Windows.Deployment.Current.Dispatcher.CheckAccess())
            {
                if (e.Error == null)
                {
                    ...
                    if (error == null)
                    {
                        SearchResultList = new ObservableCollection<Order>
                            (from order in xParse.Descendants("Order") // xParse.Element("Bid").Elements("Order")
                            select new Order()
                            {
                                ToXML = order,
                            });
