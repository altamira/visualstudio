...
begin
  ...
  from produto_fechamento pf
    inner join Metodo_Valoracao mv          on pf.cd_metodo_valoracao    = mv.cd_metodo_valoracao     
  ...
  begin                      
    ...
    from Nota_Entrada_Peps nep 
      ...
      left outer join Grupo_Produto_Custo gpc on gpc.cd_grupo_produto      = gp.cd_grupo_produto
      left outer join Unidade_Medida um       on um.cd_unidade_medida      = gpc.cd_unidade_valoracao
    ...
    begin
      ...
      select 999999999, @cd_produto, 0, 'INVALIDA', 0, @qt_saldo_produto, 0, 0, @qt_saldo_produto, 0, 0, 0, 0, getdate(), 
        ...
        'S'
      from Produto c 
        left outer join Grupo_Produto gp        on   c.cd_grupo_produto     = gp.cd_grupo_produto
        left outer join Unidade_Medida um       on   c.cd_unidade_medida    = um.cd_unidade_medida
        left outer join Grupo_Produto_Custo gpc on gpc.cd_grupo_produto     = gp.cd_grupo_produto
        left outer join Unidade_Medida umv      on umv.cd_unidade_medida    = gpc.cd_unidade_valoracao
    ...
    begin    
      ...
      from Nota_Entrada_Peps a 
        left outer join Movimento_Estoque mv    on   a.cd_movimento_estoque = mv.cd_movimento_estoque and
                                                     mv.cd_fase_produto     = @cd_fase 
        left outer join Fornecedor b            on   a.cd_fornecedor        = b.cd_fornecedor
        left outer join Produto c               on   a.cd_produto           = c.cd_produto
        left outer join Produto_Custo pc        on   a.cd_produto           = pc.cd_produto
        left outer join Materia_Prima mp        on   pc.cd_mat_prima        = mp.cd_mat_prima
        left outer join Grupo_Produto gp        on   c.cd_grupo_produto     = gp.cd_grupo_produto
        left outer join Unidade_Medida um       on   c.cd_unidade_medida    = um.cd_unidade_medida
        left outer join Grupo_Produto_Custo gpc on gpc.cd_grupo_produto     = gp.cd_grupo_produto
        left outer join Unidade_Medida umv      on umv.cd_unidade_medida    = gpc.cd_unidade_valoracao
  ...
  begin
    ...
    begin
      ...
      from #TEMP_PEPS tp
        left outer join Fase_Produto fp on fp.cd_fase_produto = tp.cd_fase_produto
...
begin
  ...
  from Nota_Saida_Item nsi 
    inner join Nota_Saida ns on nsi.cd_nota_saida = ns.cd_nota_saida 
    inner join Pedido_Venda_Item pvi on nsi.cd_pedido_venda = pvi.cd_pedido_venda and
                                        nsi.cd_item_pedido_venda = pvi.cd_item_pedido_venda
    inner join Operacao_Fiscal op on nsi.cd_operacao_fiscal = op.cd_operacao_fiscal
    inner join Grupo_Produto gp on pvi.cd_grupo_produto = gp.cd_grupo_produto
    inner join Grupo_Produto_Custo gpc on pvi.cd_grupo_produto = gpc.cd_grupo_produto 
  where ((nsi.dt_nota_saida > @dt_final) or (isnull(nsi.dt_cancel_item_nota_saida, dt_restricao_item_nota) > @dt_final)) and
  ...
  begin
    ...
    select 
      ...
      isnull(pvi.nm_produto_pedido,'') as Produto,
    ...
    from Pedido_Venda_Item_Historico_Custo pvihc
      inner join Materia_Prima mp on mp.cd_mat_prima = pvihc.cd_materia_prima
      inner join Grupo_Produto gp on gp.cd_grupo_produto = pvihc.cd_grupo_produto
      left outer join Pedido_Venda_Item pvi on pvihc.cd_pedido_venda = pvi.cd_pedido_venda and
    ...
    select 
      ...
      (sum(nei.qt_pesbru_nota_entrada) / 
        ...
        (pvi.qt_saldo_pedido_venda + isnull(nsf.qt_item_faturado,0) - isnull(nsd.qt_item_devolvido,0))) as PesoBruto,
      mp.nm_fantasia_mat_prima as MP,
      mp.nm_mat_prima as NomeMP,
      ...
      case when (op.ic_imp_operacao_fiscal = 'S') then
        ...
        sum(isnull(nei.vl_total_nota_entr_item,0) *
         ...
         pvi.qt_item_pedido_venda *
        (pvi.qt_saldo_pedido_venda + isnull(nsf.qt_item_faturado,0) - isnull(nsd.qt_item_devolvido,0)) as decimal(25,2)) as CustoTotal
    from Nota_Entrada_Item nei 
      inner join Pedido_Compra_Item pci on nei.cd_pedido_compra = pci.cd_pedido_compra and
                                           nei.cd_item_pedido_compra = pci.cd_item_pedido_compra
      inner join Pedido_Venda_Item pvi on pci.cd_pedido_venda = pvi.cd_pedido_venda and
                                          pci.cd_item_pedido_venda = pvi.cd_item_pedido_venda
      inner join Pedido_Venda pv on pvi.cd_pedido_venda = pv.cd_pedido_venda
      inner join Materia_Prima mp on pci.cd_materia_prima = mp.cd_mat_prima
      inner join Operacao_Fiscal op on nei.cd_operacao_fiscal = op.cd_operacao_fiscal
      inner join Grupo_Produto gp on gp.cd_grupo_produto = pvi.cd_grupo_produto
      inner join Grupo_Produto_Custo gpc on gpc.cd_grupo_produto = pvi.cd_grupo_produto
    ...
    where 
      ...
      ((pvi.qt_saldo_pedido_venda + isnull(nsf.qt_item_faturado,0) - isnull(nsd.qt_item_devolvido,0)) > 0) and
    ...
    group by pci.cd_pedido_venda, pci.cd_item_pedido_venda, pvi.nm_produto_pedido, 
      ...
      pvi.qt_item_pedido_venda, pvi.qt_saldo_pedido_venda, nsf.qt_item_faturado, nsd.qt_item_devolvido,
      gp.nm_grupo_produto
...
-------------------------------------------------------------------------------
if @ic_parametro = 4 -- PROCESSAMENTO DO 70 % DO PREÇO DE VENDA - PEDIDOS DE VENDA 
-------------------------------------------------------------------------------
begin

  -- NOTAS FISCAIS FATURADAS ATÉ O PERÍODO FINAL
  select nsi.cd_pedido_venda, nsi.cd_item_pedido_venda, 
    sum(nsi.qt_item_nota_saida - (case when nsi.dt_cancel_item_nota_saida is null then
                                    isnull(nsi.qt_devolucao_item_nota,0) 
                                  else
  ...
  into #NotaSaidaPrecoNacionalFaturado
  from Nota_Saida_Item nsi 
    inner join Nota_Saida ns on nsi.cd_nota_saida = ns.cd_nota_saida 
    inner join Pedido_Venda_Item pvi on nsi.cd_pedido_venda = pvi.cd_pedido_venda and
                                        nsi.cd_item_pedido_venda = pvi.cd_item_pedido_venda
  ...
  into #NotaSaidaPrecoNacionalDevolvido
  from Nota_Saida_Item nsi 
    inner join Nota_Saida ns on nsi.cd_nota_saida = ns.cd_nota_saida 
    inner join Pedido_Venda_Item pvi on nsi.cd_pedido_venda = pvi.cd_pedido_venda and
                                        nsi.cd_item_pedido_venda = pvi.cd_item_pedido_venda
  ...
  begin
    ...
    from Pedido_Venda_Item_Historico_Custo pvihc
      inner join Materia_Prima mp on mp.cd_mat_prima = pvihc.cd_materia_prima
      inner join Grupo_Produto gp on gp.cd_grupo_produto = pvihc.cd_grupo_produto
      inner join Grupo_Inventario gi on gi.cd_grupo_inventario = pvihc.cd_grupo_inventario
      left outer join Pedido_Venda_Item pvi on pvihc.cd_pedido_venda = pvi.cd_pedido_venda and
                                               pvihc.cd_item_pedido_venda = pvi.cd_item_pedido_venda
    where pvihc.dt_apuracao_custo <= @dt_final and
      isnull(pvihc.dt_baixa_estoque, @dt_final) >= @dt_final and
    ...
    from Pedido_Venda_Item_Historico_Custo pvihc
      ...
      left outer join Pedido_Venda_Item pvi on pvihc.cd_pedido_venda = pvi.cd_pedido_venda and
                                               pvihc.cd_item_pedido_venda = pvi.cd_item_pedido_venda
      left outer join Pedido_Venda pv on pv.cd_pedido_venda = pvihc.cd_pedido_venda
    ...
    select
      ...
      cast(case when isnull(pvi.vl_unitario_item_pedido, 0.00) between 0.00 and 0.01 
        ...
        (pvi.qt_saldo_pedido_venda + isnull(nsf.qt_item_faturado,0) - isnull(nsd.qt_item_devolvido,0)) as decimal(25,2)) as Preco
    from Nota_Entrada_Item nei 
      ...
      inner join Grupo_Produto gp on gp.cd_grupo_produto = pvi.cd_grupo_produto
      inner join Grupo_Produto_Custo gpc on gpc.cd_grupo_produto = pvi.cd_grupo_produto
      inner join Tipo_Pedido tp on pv.cd_tipo_pedido = tp.cd_tipo_pedido
      inner join Grupo_Inventario gi on gi.cd_grupo_inventario = 1
      left outer join #NotaSaidaPrecoNacionalFaturado nsf on nsf.cd_pedido_venda = pvi.cd_pedido_venda and
                                                             nsf.cd_item_pedido_venda = pvi.cd_item_pedido_venda
      left outer join #NotaSaidaPrecoNacionalDevolvido nsd on nsd.cd_pedido_venda = pvi.cd_pedido_venda and
                                                             nsd.cd_item_pedido_venda = pvi.cd_item_pedido_venda
    where 
      ...
      pvi.cd_pedido_venda not in (select distinct cd_pedido_venda 
                                  from Pedido_Venda_Item_Historico_Custo
    ...
    group by gi.nm_grupo_inventario, pci.cd_pedido_venda, pci.cd_item_pedido_venda, pvi.nm_produto_pedido, 
      ...
      pvi.vl_lista_item_pedido, pvi.qt_item_pedido_venda, pvi.qt_saldo_pedido_venda, 
      nsf.qt_item_faturado, nsd.qt_item_devolvido
    ...
    drop table #NotaSaidaPrecoNacionalFaturado
    drop table #NotaSaidaPrecoNacionalDevolvido
...
begin
  ...
  from Nota_Saida_Item nsi 
    inner join Nota_Saida ns on nsi.cd_nota_saida = ns.cd_nota_saida 
    inner join Operacao_Fiscal op on nsi.cd_operacao_fiscal = op.cd_operacao_fiscal
    inner join Grupo_Produto gp on nsi.cd_grupo_produto = gp.cd_grupo_produto
  ...
  from Nota_Entrada_Item nei 
    ...
    inner join Pedido_Venda_Item pvi on pci.cd_pedido_venda = pvi.cd_pedido_venda and
                                        pci.cd_item_pedido_venda = pvi.cd_item_pedido_venda
    inner join Pedido_Venda pv on pvi.cd_pedido_venda = pv.cd_pedido_venda
    inner join Fornecedor f on nei.cd_fornecedor = f.cd_fornecedor 
    inner join Materia_Prima mp on pci.cd_materia_prima = mp.cd_mat_prima
    inner join Operacao_Fiscal op on nei.cd_operacao_fiscal = op.cd_operacao_fiscal
    inner join Grupo_Produto gp on gp.cd_grupo_produto = pvi.cd_grupo_produto
    inner join Grupo_Produto_Custo gpc on gpc.cd_grupo_produto = pvi.cd_grupo_produto
    left outer join #NotaSaidaPV ns on ns.cd_pedido_venda = pvi.cd_pedido_venda and
...
begin
  ...
  from Nota_Saida_Item nsi 
    ...
    inner join Operacao_Fiscal op on nsi.cd_operacao_fiscal = op.cd_operacao_fiscal
    inner join Grupo_Produto gp on nsi.cd_grupo_produto = gp.cd_grupo_produto
    inner join Grupo_Produto_Custo gpc on nsi.cd_grupo_produto = gpc.cd_grupo_produto 
  where nsi.dt_cancel_item_nota_saida is null and
    gp.ic_especial_grupo_produto = 'S' and
    gpc.ic_custo = 'S' and
    op.ic_estoque_op_fiscal = 'S' and
  ...
  from Nota_Entrada_Item nei 
    inner join Nota_Entrada ne on nei.cd_nota_entrada = ne.cd_nota_entrada and
                                  ...
                                  nei.cd_operacao_fiscal = ne.cd_operacao_fiscal and
                                  nei.cd_serie_nota_fiscal = ne.cd_serie_nota_fiscal      
    inner join Fornecedor f on f.cd_fornecedor = nei.cd_fornecedor 
    inner join Pedido_Compra_Item pci on pci.cd_pedido_compra = nei.cd_pedido_compra and
                                         pci.cd_item_pedido_compra = nei.cd_item_pedido_compra
    inner join Materia_Prima mp on pci.cd_materia_prima = mp.cd_mat_prima
    inner join Pedido_Venda_Item pvi on pvi.cd_pedido_venda = pci.cd_pedido_venda and
    ...
    inner join Grupo_Produto_Custo gpc on gpc.cd_grupo_produto = pvi.cd_grupo_produto
    inner join Tipo_Pedido tp on pv.cd_tipo_pedido = tp.cd_tipo_pedido
    left outer join #NotaSaidaPrecoNacionalPV ns on ns.cd_pedido_venda = pvi.cd_pedido_venda and
                                                    ns.cd_item_pedido_venda = pvi.cd_item_pedido_venda
  where 
    -- QUE TENHAM SIDO FABRICADOS ATÉ O MÊS
    (pvi.dt_entrega_fabrica_pedido <= @dt_final) and  
...
begin
  ...
  from Produto_Fechamento pf
    ...
    inner join Produto_Custo pc on pc.cd_produto = pf.cd_produto
    inner join Fase_Produto fp on pf.cd_fase_produto = fp.cd_fase_produto
    inner join Materia_Prima mp on pc.cd_mat_prima = mp.cd_mat_prima
    inner join Metodo_Valoracao mv on pf.cd_metodo_valoracao = mv.cd_metodo_valoracao
    left outer join Bitola_Maior_Custo_Historico bmch on pc.cd_bitola = bmch.cd_bitola and
  ...
  begin
    ...
    from Produto_Custo pc
      inner join #ProdutoPadraoCompra custo on custo.CodProduto = pc.cd_produto and
    ...
    from Produto_Fechamento pf
      inner join #ProdutoPadraoCompra custo on custo.CodProduto = pf.cd_produto and
...
begin
  ...
  select pf.cd_fase_produto as CodFaseProduto,
    ...
    isnull((select top 1 cast(cd_nota_saida as varchar)
     from Nota_Saida_Item nsi, Operacao_Fiscal op
     where nsi.cd_produto = p.cd_produto and
       cast(nsi.vl_unitario_item_nota as decimal(25,2)) = 
            (select cast(max(isnull(aux.vl_unitario_item_nota,0)) as decimal(25,2)) 
             from nota_saida_item aux, operacao_fiscal op
  ...
  from Produto_Fechamento pf
    ...
    inner join Grupo_Produto gp on p.cd_grupo_produto = gp.cd_grupo_produto
    inner join Grupo_Produto_Custo gpc on p.cd_grupo_produto = gpc.cd_grupo_produto
  where pf.dt_produto_fechamento = @dt_custo and
    pf.qt_atual_prod_fechamento > 0 and
