...
set @qt_dia_imediato = (select qt_dia_imediato_empresa
  ...
  from
    Consulta           co inner join 
    Consulta_Itens    ico   on ico.cd_consulta          = co.cd_consulta left outer join
    Cliente             c   on c.cd_cliente             = co.cd_cliente left outer join
    Condicao_Pagamento pg   on pg.cd_condicao_pagamento = co.cd_condicao_pagamento left outer join
    Categoria_Produto  cp   on cp.cd_categoria_produto  = ico.cd_categoria_produto left outer join
    Vendedor            v   on v.cd_vendedor            = co.cd_vendedor left outer join 
    egisadmin.dbo.Usuario u on co.cd_usuario_atendente  = u.cd_usuario 
