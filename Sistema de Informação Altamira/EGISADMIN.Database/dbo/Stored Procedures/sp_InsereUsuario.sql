...
BEGIN
  ...
  SELECT @cd_usuario = ISNULL(MAX(cd_usuario),0) + 1 FROM Usuario TABLOCK
  SET @nm_fantasia_usuario = RTrim(LTrim(@nm_fantasia_usuario))
  SET @cd_senha_usuario = RTrim(LTrim(@cd_senha_usuario))
  INSERT INTO Usuario 
            ( cd_usuario,
              nm_usuario,
              nm_fantasia_usuario,
              ...
              cd_telefone_usuario,
              cd_celular_usuario,
              nm_arquivo_foto_usuario,
              dt_cadastro_usuario,
              nm_local_arquivo_usuario,
              qt_dias_troca,
              ic_consulta_executiva,
              ic_consulta_comissao_total,
              cd_vendedor,
	      ic_alterar_primeiro_log,
	      ic_ativo,
	      cd_departamento,
	      ic_moeda_usuario,
              ic_lista_aniversariantes,
	      ic_wapnet_usuario,
	      cd_senha_wapnet_usuario,
	      ic_repnet_usuario,
	      cd_senha_repnet_usuario,
	      cd_senha_email,
	      nm_usuario_email,
	      nm_assinatura_usuario,
	      ic_dica_usuario,
	      cd_cargo_empresa,
              ic_lembrete_usuario,
	      ic_ass_eletronica_usuario,
	      ic_operador_tmkt_usuario,	      
	      ic_acesso_internet_usuario,
              cd_usuario_atualiza,
              dt_usuario_atualiza,
              cd_comprador,
              ic_ocorrencia_usuario,
	      cd_tipo_aprovacao,
	      ic_bx_req_interna_usuario,
              cd_idioma,
              ic_multi_idioma_usuario,
              ic_autoriza_pagamento, 
              cd_loja,
              ic_instrucao_usuario,
              cd_centro_custo,
              cd_projetista,
              ic_exportacao_usuario,
              ic_email_usuario,
              ic_acesso_padrao_usuario,
              ic_bx_req_fab_usuario,
              cd_funcionario,
              ic_moeda_nacional,
              ic_moeda_estrangeira,
              ic_bloqueio_vendedor)
     VALUES (@cd_usuario,
             @nm_usuario,
             @nm_fantasia_usuario,
             @cd_senha_usuario,
  ...
  INSERT INTO Historico_Senha_Usuario  (dt_troca_senha,cd_usuario,cd_senha_usuario, cd_usuario_atualiza, dt_atualiza)
       VALUES (@dt_cadastro_usuario,@cd_usuario,@cd_senha_usuario, @cd_usuario_atualiza, @dt_usuario_atualiza)
  if @@ERROR <> 0 
    Goto TrataErro
  COMMIT TRAN
