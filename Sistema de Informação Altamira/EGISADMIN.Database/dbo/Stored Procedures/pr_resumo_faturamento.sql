...
--    cast(sum(vw.vl_unitario_item_total)  as money )                             as Venda,
...
--select * from #FaturaDevolucaoAnoAnterior
  ...
  select 
    ...
    cast(sum( case when isnull(vw.vl_unitario_item_atual,0)>0 then vw.vl_unitario_item_atual else vw.vl_total end

             ) / @vl_moeda as money )					                                as Venda,
...
--                order by vm.dt_moeda desc),1)                                              as cotacao 
  ...
  from 
    ...
    left outer join #FaturaDevolucao            b with (nolock) on a.dt_nota_saida           = b.dt_nota_saida and
                                                                   a.cd_tipo_operacao_fiscal = b.cd_tipo_operacao_fiscal and 
                                                                   a.cd_cliente_grupo        = b.cd_cliente_grupo

    left outer join #FaturaDevolucaoAnoAnterior c with (nolock) on a.dt_nota_saida           = c.dt_restricao_item_nota and
                                                                   a.cd_tipo_operacao_fiscal = c.cd_tipo_operacao_fiscal and 
                                                                   a.cd_cliente_grupo        = c.cd_cliente_grupo

    left outer join #FaturaCancelado            d with (nolock) on a.dt_nota_saida           = d.dt_nota_saida           and
                                                                   a.cd_tipo_operacao_fiscal = d.cd_tipo_operacao_fiscal and 
                                                                   a.cd_cliente_grupo        = d.cd_cliente_grupo
