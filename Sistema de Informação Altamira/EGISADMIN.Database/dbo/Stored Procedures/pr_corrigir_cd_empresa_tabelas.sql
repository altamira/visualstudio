...
WHERE isnull(ic_implantacao_tabela,'N') = 'S' AND  
      isnull(ic_inativa_tabela, 'N') = 'N' AND  
  	   isnull(ic_sap_admin, 'N') = 'N'  and isnull(t.cd_banco_dados,0)  = 2 and
      @ic_parametro = 1
...
-- union all
-- select 'Delete from EGISADMIN.dbo.Empresa where cd_empresa = '+Cast(@cd_empresa_atual as varchar(5)) where  @ic_parametro = 1
-- union all
-- Select 'Go'
-------------------------------------------------------------------------------------------------------------------------
