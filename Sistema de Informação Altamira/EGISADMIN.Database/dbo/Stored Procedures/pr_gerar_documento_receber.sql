...
------------------------------------------------------------------------------------------------------------------------------------
@ic_parametro 	int,
@cd_nota_saida  int, --Irá receber também o pedido de venda quando geração for pelo pedido
@dt_inicial	datetime,
@dt_final	datetime,
@cd_usuario	int
...
declare @ic_rateio int
  ...
  select
    ...
    @ic_centro_custo_scr = ic_centro_custo_scr 
  from  
...
--  					                               where y.cd_documento_receber = x.cd_documento_receber) )
    ...
    where 
      ...
      cd_documento_receber not in (select cd_documento_receber from documento_receber_pagamento
    ...
    from
		  ...
		  inner Join
		  Movimento_Caixa_Pedido mped
		  on mped.cd_pedido_venda = ns.cd_pedido_venda

      inner Join
      Movimento_Caixa mc
      on mc.cd_movimento_caixa = mped.cd_movimento_caixa
		where
    ...
    from
      Nota_Saida_Parcela p
        left outer join
      Nota_Saida n
        on p.cd_nota_saida = n.cd_nota_saida
        left outer join  
      Operacao_Fiscal o
        on n.cd_operacao_fiscal = o.cd_operacao_fiscal
        left outer join
      Parametro_Financeiro f
        on f.cd_empresa = dbo.fn_empresa()
        left outer join
      vw_destinatario d
...
-- -- --           , @ic_baixa_autom_scr       = ic_baixa_autom_scr
        ...
        if ( @vl_credito_devolucao > 0 ) begin
          ...
          set @ds_documento_receber = 'Geração Automática p/ Faturamento.'
        end
  ...
  begin
    ...
    from
      ...
      Parametro_Financeiro f
        ...
        left outer join
      vw_destinatario d
        on n.cd_cliente = d.cd_destinatario and
    ...
    where
      -- ELIAS 29/03/2004
      (isnull(p.vl_parcela_nota_saida,0)<>0) and
      p.ic_scr_parcela_nota_saida <> 'S' and
...
-----------------------------------------------------------------------------------------
  begin
    ...
    set @ic_identificacao_parcela = IsNull(( select top 1 ic_identificacao_parcela 
    ...
    from
      Pedido_Venda_Parcela pvp left outer join
      Pedido_Venda         pv  on pvp.cd_pedido_venda = pv.cd_pedido_venda left outer join
      Parametro_Comercial  pc  on pc.cd_empresa = dbo.fn_empresa()
      left outer join Cliente_Informacao_Credito cic on cic.cd_cliente = pv.cd_cliente
    where
