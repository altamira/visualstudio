...
-------------------------------------------------------------------------------
--sp_helptext pr_geracao_financeiro_calculo_folha_sintetico
-------------------------------------------------------------------------------
--pr_geracao_financeiro_calculo_folha_sintetico
-------------------------------------------------------------------------------
...
--                   Sintético por Evento da Folha  
--Data             : 12.06.2008
--Alteração        : 
--
-- 19.1.2011 - Modificações Diversas - Carlos Fernandes
------------------------------------------------------------------------------
create procedure pr_geracao_financeiro_calculo_folha_sintetico
...
--select * from tipo_autorizacao_pagamento
  ...
  set @cd_moeda   = 1
  set @cd_tipo_ap = 13 --Por enquanto o correto é vir do cadastro de tipo de cálculo
  set @vl_ap      = 0
  ...
  select
    @cd_tipo_ap            = isnull(cd_tipo_ap,13),
  ...
  --select * from calculo_folha where cd_controle_folha = 14

  select 
    @vl_ap                 = sum ( isnull(fc.vl_total_liquido,0) ),
    @ds_ap                 = max(@nm_tipo_calculo_folha+' No.: '+cast(fc.cd_controle_folha as varchar))
...
-----------------------------------------------------------------------------------------
  ...
  set @cd_identificacao_document = isnull(@sg_identificacao,'')+'-'+cast(isnull(@cd_controle_folha,0) as varchar)
...
--  select @cd_identificacao_document,@vl_ap
  ...
  begin
     ...
     from
       Config_Folha_Documento_pagar with (nolock)
     where
