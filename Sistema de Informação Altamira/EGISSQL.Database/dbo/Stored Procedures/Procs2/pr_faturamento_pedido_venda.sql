...
--Objetivo         : Mapa de Pedidos Faturados
--Data             : 24.03.2007
...
-- 14.03.2009 - Ajustes Diversos e Complemento - Carlos Fernandes
-- 07.09.2009 - Faturamento de Serviço - ajuste diversos na procedure - Carlos Fernandes
-- 09.11.2009 - Observação de Fábrica do PCP - Pedido de Serviços - Carlos Fernandes
-- 23.10.2010 - Ajuste da Nota - Carlos Fernandes
----------------------------------------------------------------------------------------
create procedure pr_faturamento_pedido_venda
...
--  i.cd_nota_saida                                       as 'Nota',
  ...
  case when isnull(d.cd_produto,0)>0 then
     d.nm_fantasia_produto
  else
    case when isnull(d.cd_servico,0)>0 then
      s.sg_servico
    else
      'Especial'
    end
  end                                                   as nm_fantasia_produto,
  case when isnull(nm_produto_pedido,'')<>'' then
    d.nm_produto_pedido                               
  else
    case when isnull(d.cd_servico,0)>0 then
       s.nm_servico
...
--       Null end,
  ...
  MascaraProduto =
    case when isnull(d.cd_servico,0)>0 then
      ...
      case when (a.cd_tipo_pedido = 1) or (d.cd_produto > 0) then
        (select max(cd_mascara_produto) 
         from Produto where cd_produto = d.cd_produto)
  ...
  Atendimento = ( case when n.dt_nota_saida <= d.dt_entrega_vendas_pedido then 'S' else 'N' end ),
...
from 
  ...
  left Join Servico s             with (nolock) on s.cd_servico             = d.cd_servico
