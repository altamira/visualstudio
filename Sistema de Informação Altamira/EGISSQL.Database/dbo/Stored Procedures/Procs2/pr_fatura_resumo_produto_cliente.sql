...
CREATE PROCEDURE pr_fatura_resumo_produto_cliente
---------------------------------------------------
...
as
  ...
  Select 
  	vw.cd_produto,
  	IsNull(vw.cd_cliente,0) cd_cliente,
  ...
  where 
    ...
    and (vw.cd_cliente = @cd_cliente or @cd_cliente = 0) 
  group by 
  	IsNull(vw.cd_cliente,0),
  	vw.cd_produto
  order by 1, 2 desc
  ...
  from 
    #FaturaAnual a
      left outer join  
    #FaturaDevolucao b
  	  on a.cd_cliente = b.cd_cliente and a.cd_produto = b.cd_produto
      left outer join  
    #FaturaDevolucaoAnoAnterior c
  	  on a.cd_cliente = c.cd_cliente  and a.cd_produto = c.cd_produto
      left outer join  
    #FaturaCancelado d
  	  on a.cd_cliente = d.cd_cliente  and a.cd_produto = d.cd_produto
