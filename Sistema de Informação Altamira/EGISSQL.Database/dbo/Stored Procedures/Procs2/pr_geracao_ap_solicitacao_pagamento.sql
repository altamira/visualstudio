...
-----------------------------------------------------------------------------------------
--sp_helptext pr_geracao_ap_solicitacao_pagamento
-----------------------------------------------------------------------------------------
...
--Objetivo       : Geração de Autorização de Pagamento para Solicitação de Pagamento
--Data           : 25.04.2006
...
-- 09.11.2007 - Composição da Autorização com os Dados da Solicitação - Carlos Fernandes
-- 26.12.2007 - Complemento dos campos - Carlos Fernandes
-- 20.06.2008 - 
-- 04.12.2008 - Pedido de Importação - Carlos Fernandes
-- 27.02.2009 - Novos campos da tabela de documento_pagar - Carlos Fernandes
-----------------------------------------------------------------------------------------

CREATE PROCEDURE pr_geracao_ap_solicitacao_pagamento
...
--select * from tipo_autorizacao_pagamento
  ...
  select 
    @vl_ap                 = isnull(vl_pagamento,0),
    @ds_ap                 = 'Solicitação Pagamento No.: '+cast(cd_solicitacao as varchar),
  ...
  from
    solicitacao_pagamento with (nolock) 
  where
    cd_solicitacao = @cd_solicitacao 
...
--select * from tipo_autorizacao_pagamento
  ...
  --composicao
   ...
   select
     ...
     cd_item_solicitacao                    as cd_item_ap,
     null                                   as cd_tipo_documento,
     cd_solicitacao                         as cd_documento_ap,
     @cd_usuario                            as cd_usuario,
     getdate()                              as dt_usuario,
     'A'                                    as ic_tipo_documento_ap,
     'SP-'+cast(@cd_solicitacao as varchar) as cd_identificacao_documento
   into
     #autorizacao_pagto_composicao
   from
     solicitacao_pagamento_composicao
   where
     cd_solicitacao = @cd_solicitacao 
...
-----------------------------------------------------------------------------------------
  ...
  set @cd_identificacao_document = 'SP-'+cast(@cd_solicitacao as varchar)
  ...
  begin
    ...
    select
      ...
      'SP-'+cast(@cd_solicitacao as varchar)       as cd_identificacao_document,
