...
create procedure pr_diario_exportacao

------------------------------------------------------------------------
--pr_diario_exportacao
------------------------------------------------------------------------
...
set @qt_dia_imediato = ( select qt_dia_imediato_empresa 
  ...
  select
    ...
    tm.nm_tipo_mercado
  from
    vw_exportacao_venda_bi vw left outer join
    Condicao_Pagamento pg on vw.cd_condicao_pagamento = pg.cd_condicao_pagamento left outer join
    Tipo_Mercado tm on tm.cd_tipo_mercado = vw.cd_tipo_mercado
  where
