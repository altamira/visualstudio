create procedure pr_ranking_regiao_vendedor_faturamento_teste
-----------------------------------------------------------------------------------
...
begin
  ...
  select 
    ...
    isnull(vl.cd_loja,0) as cd_loja,
  ...
  from   
    #FaturaResultado a
      Left Outer Join
    Vendedor vl
      on IsNull(a.cd_vendedor,0) = IsNull(vl.cd_vendedor,0)
  Group By
    IsNull(vl.cd_loja,0)
  order by venda desc
  ...
  select 
    a.*,
    IsNull(lj.nm_loja,'(Sem Loja)') nm_loja
  from 
    #FaturaSetorAux a left outer join
    Loja lj on IsNull(a.cd_loja,0) = IsNull(lj.cd_loja,0)
  order by 
    posicao
...
begin
  ...
  from
    ...
    Vendedor vr on vr.cd_loja = @cd_loja
  ...
  from 
    ...
    Vendedor vr on IsNull(a.cd_vendedor,0) = IsNull(vr.cd_vendedor,0)
                          and vr.cd_loja = @cd_loja        
  order by 
    a.venda desc
