...
CREATE PROCEDURE pr_fatura_cidade
------------------------------------------------------------------------------------------------------
...
as
  ...
  Select 
    ...
    IsNull(vwd.cd_cidade,0)                    as cd_cidade,
...
--  	(vw.dt_nota_saida < @dt_inicial) and
   vw.dt_nota_saida < @dt_inicio_periodo 
  ...
  from
    #FaturaResultado a 
    left outer join  Pais c    on a.cd_pais    = c.cd_pais
    left outer join  Estado b  on a.cd_pais    = b.cd_pais and a.cd_estado = b.cd_estado
    left outer join  Cidade ci on ci.cd_pais   = a.cd_pais     and
                                  ci.cd_estado = a.cd_estado and
                                  ci.cd_cidade = a.cd_cidade 
  Order by Venda desc
