...
-------------------------------------------------------------------------------
--sp_helptext pr_liberacao_prestacao_conta_fornecedor
-------------------------------------------------------------------------------
...
-- 09.06.2008 - Liberação apenas das prestações de contas para Fornecedor - Carlos Fernandes
-- 01.09.2008 - Data do Vencimento - Carlos Fernandes
--------------------------------------------------------------------------------------------
create procedure pr_liberacao_prestacao_conta_fornecedor
@ic_parametro int      = 0,
@dt_inicial   datetime = '',
@dt_final     datetime = '',
@ic_selecao   int      = 0
...
--    left join Prestacao_Conta_Composicao pcc with (nolock) on pc_prestacao          = pcc.cd_prestacao
...
--    isnull(pc.cd_cartao_credito,0) = 0 and
    ...
    pc.dt_liberacao_fornecedor     is null     and
    isnull(f.cd_conta,0)>0                     and
    pc.dt_pagamento_prestacao      is null
...
begin
  ...
  --Atualiza a prestacao de contas

  update
    prestacao_conta
  set
    dt_conf_fornecedor     = null,
    dt_pagamento_prestacao = null
  from
    prestacao_conta p
  where
    p.cd_prestacao in (select cd_prestacao from #LiberacaoFornecedor )
  ...
  set
    ic_contabilizado_fornecedor = 'N'
  where
    cd_prestacao in (select cd_prestacao from #LiberacaoFornecedor )


  --Mostra os registros para o usuário
