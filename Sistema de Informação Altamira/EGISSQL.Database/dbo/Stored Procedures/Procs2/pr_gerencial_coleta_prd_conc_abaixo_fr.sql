...
/****** Object:  Stored Procedure dbo.pr_gerencial_coleta_prd_conc_abaixo_fr    Script Date: 13/12/2002 15:08:32 ******/





CREATE     PROCEDURE pr_gerencial_coleta_prd_conc_abaixo_fr
--pr_gerencial_coleta_prd_conc_abaixo_fr
--------------------------------------------------------------------------------------
...
select *, cast(cast(visitas as decimal(16,2))/ cast(lojas as decimal(16,2)) * 100 as decimal(16,2)) as 'Penetracao',
          cast(  (cast(PrcMed as Decimal(16,2))/cast(VlrMedioPadrao as Decimal(16,2)))*100 as decimal(16,2)) as 'PosPreco'
into #ResultadoFinal
from Resultado3

Set @SQL = 'select '+
'loj.nm_loja_coleta as Loja, '+
'rc.nm_regiao_coleta as Localizacao, '+
'est.nm_estado as Estado, '+
'cp.nm_categoria_produto as Categoria, '+
'p.nm_marca_produto as Marca, '+
'p.nm_Fantasia_produto Produto, '+
'cast(isnull(coi.qt_frente_item_coleta,0) as decimal(16,0)) as Campo, '+
'coi.cd_status_coleta Status '+
'into LojasPreco '+
'from  '+
'produto p, '+
'categoria_produto cp, '+
...
'co.cd_coleta = coi.cd_coleta and '+
'p.cd_produto = coi.cd_produto and '+
'rc.cd_regiao_coleta = co.cd_regiao_coleta and '

if @Estado = 'S'
   set @SQL = @SQL + 'rc.cd_estado = '+@cd_Estado+' and '

set @SQL = @SQL + 'rc.cd_estado = est.cd_estado and  '+
'coi.qt_frente_item_coleta > 0'
exec(@SQL)


Select Distinct lp.*, rf.FrentesMed as CampoMed from lojasPreco lp
left outer join #ResultadoFInal rf
on ((rf.marca=lp.marca) and (rf.produto=lp.produto))
where lp.Campo < (rf.FrentesMed-(rf.FrentesMed*(@tolerancia/100)))
order by Loja,Localizacao 
