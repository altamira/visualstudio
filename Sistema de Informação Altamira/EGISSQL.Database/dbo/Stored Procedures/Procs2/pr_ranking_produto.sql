...
Create procedure pr_ranking_produto
-----------------------------------------------------------------------------------  
...
--     case when isnull(a.cd_servico,0)>0 
--          then a.cd_servico else a.cd_produto end                         as 'Produto',   
    ...
    max(a.nm_familia_produto)                  as nm_familia_produto,
    max(a.nm_grupo_produto)                    as nm_grupo_produto,
    max(a.nm_marca_produto)                    as nm_marca_produto  
...
--    a.cd_produto, a.nm_fantasia_produto  
  ...
  from #VendaCateg a left outer join  
       Produto b on   a.produto = b.cd_produto left outer join  
       Unidade_medida un on un.cd_unidade_medida = b.cd_unidade_medida  left outer join
		 Categoria_Produto cp ON(a.cd_categoria_produto = cp.cd_categoria_produto)
  order by posicao  
