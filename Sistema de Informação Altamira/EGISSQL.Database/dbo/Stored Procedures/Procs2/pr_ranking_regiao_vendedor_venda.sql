...
CREATE procedure pr_ranking_regiao_vendedor_venda 
-----------------------------------------------------------------------------------
...
begin
  ...
  from
    vw_venda_bi vw 
  left outer join Vendedor_Regiao vr on
...
--Cria a Tabela Final de Vendas por Setor
  select IDENTITY(int, 1,1)      as 'Posicao',
         a.cd_regiao, 
         (Select top 1 nm_regiao_venda from regiao_venda where cd_regiao_venda = a.cd_regiao) as nm_regiao,
         isnull((Select qtdvendedores from #Resumo_Vendedor_Setor where cd_regiao = a.cd_regiao),0) as Qtd_vendedores,
...
begin
  ...
  from
    Cliente c
  inner join Vendedor_Regiao vr on
    c.cd_vendedor=vr.cd_vendedor and
    vr.cd_regiao_venda=@cd_regiao
  group by
    c.cd_vendedor
...
-- Geração da Tabela Auxiliar de Vendas por Cliente
  select 
    vr.cd_regiao_venda as 'cd_regiao', 
...
--    IsNull(a.dt_cancelamento_pedido,@dt_final+1) > @dt_final --Desconsider pedidos de venda cancelados                
  group by 
    vr.cd_regiao_venda, vw.cd_categoria_produto
  order by 3 desc 
