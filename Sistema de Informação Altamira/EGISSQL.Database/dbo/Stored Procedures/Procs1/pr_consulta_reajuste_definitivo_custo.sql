...
CREATE PROCEDURE pr_consulta_reajuste_definitivo_custo
@ic_parametro          int,
@cd_produto            int,
...
begin
  ...
  select
    ...
    p.nm_produto,
    isnull(pc.vl_custo_produto,0)        as vl_produto,
    u.sg_unidade_medida,
    pp.dt_produto_preco,
    ...
    pp.cd_usuario,
    pp.dt_usuario
  from Produto_Preco_Custo pp        with (nolock)
...
begin
  ...
  select
    ...
    @cd_moeda             = pc.cd_moeda,
    @vl_produto           = pc.vl_custo_produto,
    @cd_motivo_reajuste   = pp.cd_motivo_reajuste
  from
    Produto p                         with (nolock)
    inner join Produto_Custo pc       with (nolock) on pc.cd_produto = p.cd_produto 
    inner join Produto_Preco_Custo pp with (nolock) on p.cd_produto  = pp.cd_produto 
  where
  ...
  set vl_custo_produto = @vl_temp_preco_produto
  where
    cd_produto = @cd_produto
    
  delete Produto_Preco_Custo
...
begin
  ...
  update 
    Produto_Custo
  set 
    vl_custo_produto = @vl_temp_preco_produto,
...
begin
	...
	into
		#Atualizacao_Em_Lote
	from
		Produto_Preco_Custo with (nolock)
	...
	begin
		...
		exec pr_consulta_reajuste_definitivo_custo
