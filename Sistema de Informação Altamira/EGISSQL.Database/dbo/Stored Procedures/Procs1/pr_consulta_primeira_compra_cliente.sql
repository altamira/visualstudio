...
-------------------------------------------------------------------------------
--sp_helptext pr_consulta_primeira_compra_cliente
-------------------------------------------------------------------------------
--pr_consulta_primeira_compra_cliente
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_consulta_primeira_compra_cliente
@dt_inicial datetime = '',
@dt_final   datetime = ''
...
where
  ...
  vw.dt_cancelamento_pedido is null and
  --Casos Especiais de Serviço = Produto 
  isnull(vw.cd_produto_servico,0)   = 0                
...
select
  ...
  ( select top 1 isnull(cd_pedido_venda,0)
    ...
    where 
      ...
      cd_cliente = vw.cd_cliente ) as cd_pedido_venda,
  ...
  ( select top 1 isnull(vl_total_pedido_ipi,0)
    ...
    where 
      dt_pedido_venda = vw.dt_pedido_venda and
      dt_cancelamento_pedido is null       and
      cd_cliente = vw.cd_cliente ) as vl_total_pedido_venda
