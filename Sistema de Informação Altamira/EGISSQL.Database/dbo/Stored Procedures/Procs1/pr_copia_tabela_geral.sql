...
@nm_tabela_copia            varchar(80), -- Nome da tabela a ser copiada
...
from 
  ...
  ( select c.id, c.name
    from syscolumns c 
    where 
      ( case when c.name = 'cd_usuario' then 1
...
begin
  ...
  set @SQL = @SQL + ' Update #'+@nm_name+ 
	      ...
	      @nm_campo_chave + ' = ' + cast(@cd_campo_chave_new as varchar(50)) + ', ' +
              ' cd_usuario = ' + cast(@cd_usuario as varchar) + ',' +
              ' dt_usuario = GetDate() ' 
