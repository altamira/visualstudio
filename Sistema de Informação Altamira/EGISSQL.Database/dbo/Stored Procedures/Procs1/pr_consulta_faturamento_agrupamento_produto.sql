CREATE PROCEDURE pr_consulta_faturamento_agrupamento_produto
---------------------------------------------------
...
as
  ...
  Where
    (cd_agrupamento_produto = @cd_agrupamento_produto or @cd_agrupamento_produto = 0)
  ...
  Select  
    c.cd_agrupamento_produto,
    c.sg_agrupamento_produto,
    a.*
  from 
    #FaturaResultado a
      left outer join
    produto p
      on a.cd_produto = p.cd_produto
      left outer join
    agrupamento_produto c
      on p.cd_agrupamento_produto = c.cd_agrupamento_produto
