...
CREATE PROCEDURE pr_consulta_pedido_ativado
---------------------------------------------------
--GBS - Global Business Sollution              2002
--Stored Procedure: Microsoft SQL Server       2000
--Autor(es): Anderson 
--Banco de Dados: SAPSQL
--Objetivo: Mostrar os pedidos ativados
--Data: 26/05/2004
--Atualizado: 
---------------------------------------------------
@cd_parametro int,
...
Begin
  ...
  SELECT
    p.dt_ativacao_pedido_venda,
    p.ds_ativacao_pedido,
    t.sg_tipo_pedido,
    t.nm_tipo_pedido,
    p.cd_pedido_venda,
    p.dt_pedido_venda,
  ...
  where
  (IsNull(p.dt_pedido_venda,getdate()) between 
    (case @ic_tipo_filtro when 'E' then @dt_inicial else IsNull(p.dt_pedido_venda,getdate()) end ) and 
    (case @ic_tipo_filtro when 'E' then @dt_final else IsNull(p.dt_pedido_venda,getdate()) end )
  )
  and
  (IsNull(p.dt_ativacao_pedido_venda, getdate()) between 
    (case @ic_tipo_filtro when 'A' then @dt_inicial else IsNull(p.dt_ativacao_pedido_venda, getdate()) end ) and 
    (case @ic_tipo_filtro when 'A' then @dt_final else IsNull(p.dt_ativacao_pedido_venda, getdate()) end )
  )
 and
    p.dt_ativacao_pedido_venda is not null
  order by
