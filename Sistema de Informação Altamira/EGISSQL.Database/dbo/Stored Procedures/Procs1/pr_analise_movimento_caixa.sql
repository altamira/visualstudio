...
-- 26.11.2009 - Verificação da Coluna em Atraso e o seu mecanismo de soma - Carlos Fernandes
-- 19.06.2010 - Novas Colunas CR/CP Aberto/Pagos - Carlos Fernandes
-- 20.09.2010 - Faturamento de pedido parcial - Carlos Fernandes
-- 28.11.2010 - Pedido de Vendas com Bonificação - Não pode entrar no fluxo de caixa - Carlos Fernandes
------------------------------------------------------------------------------------------------------------------
...
--            - isnull(drp.vl_credito_pendente, 0))
           ...
           cast( 0 as float)                       as vl_aberto_documento_receber, 
           cast( 0 as float)                       as vl_pago_documento_receber,   
           cast( 0 as float)                       as vl_aberto_documento_pagar,   
           cast( 0 as float)                       as vl_pago_documento_pagar
...
-- pvi.dt_entrega_vendas_pedido,
 ...
 isnull(pvi.qt_saldo_pedido_venda * pvi.vl_unitario_item_pedido,0) as vl_total,

 cp.qt_parcela_condicao_pgto,
 cpp.pc_condicao_parcela_pgto,
 cpp.qt_dia_cond_parcela_pgto,
 isnull(( pvi.qt_saldo_pedido_venda * pvi.vl_unitario_item_pedido ) * ( cpp.pc_condicao_parcela_pgto/100),0) as Parcela,
...
--  (pvi.dt_entrega_vendas_pedido 
 ...
 isnull(pv.ic_bonificacao_pedido_venda,'N')='N' and  --Bonificação
 isnull(pv.ic_amostra_pedido_venda,'N')    ='N' and  --Amostra
 pv.dt_fechamento_pedido is not null            and  --Pedido Fechado
...
(
 ...
 + 
  ...
  isnull(cpp.qt_dia_cond_parcela_pgto,0) )>@dt_inicial and         --@dt_hoje and
...
--  (pvi.dt_entrega_vendas_pedido 
 ...
 + isnull(cpp.qt_dia_cond_parcela_pgto,0) ) between @dt_inicial and @dt_final and
...
SELECT     
           ...
           cast( 0 as Float)                         as vl_saldo_previsto_saida,
           cast( 0 as float)                         as vl_desconto_documento,
           sum( isnull(Parcela,0) )                  as vl_previsao_vendas,
           cast( 0 as float )                        as vl_previsao_compras,
           cast( 0 as float)                         as vl_previsao_importacao,
           cast( 0 as float)                         as vl_atraso,
           cast( 0 as float)                         as vl_aberto_documento_receber, 
...
create table #TabInicial
 ( Vencimento                  datetime Null,
   ...
   vl_aberto_documento_receber float null,
   vl_pago_documento_receber   float null,
   vl_aberto_documento_pagar   float null,
   vl_pago_documento_pagar     float null
  )
...
--         0.00                          as vl_aberto_documento_receber, 
--         0.00                          as vl_pago_documento_receber,   
--         0.00                          as vl_aberto_documento_pagar,   
--         0.00                          as vl_pago_documento_pagar

        sum(isnull(vl_saldo_documento_receber,0))                          as vl_aberto_documento_receber, 
        sum(isnull(vl_pagamento_documento_receber,0))                      as vl_pago_documento_receber,   
        sum(isnull(vl_saldo_documento_pagar,0))                            as vl_aberto_documento_pagar,   
        sum(isnull(vl_pagamento_documento_pagar,0))                        as vl_pago_documento_pagar
...
select
  *, 
  cast(0.00 as Float) as 'SaldoAcumulado' 
into 
  #Tab_Aux 
from 
  #TabInicial
...
select top 1
  ...
  cast(0.00 as float)           as vl_saldo_documento_receber,
  cast(0.00 as float)           as vl_saldo_documento_pagar,
  cast(0.00 as float)           as vl_saldo_previsto_entrada,
  cast(0.00 as float)           as vl_saldo_previsto_saida,
  cast(0.00 as float)           as vl_desconto_documento,
  cast(0.00 as float)           as vl_saldo_previsao_vendas,
  cast(0.00 as float)           as vl_saldo_previsao_compras,
  cast(0.00 as float)           as vl_saldo_previsao_importacao,
  cast(0.00 as float)           as vl_atraso,
  isnull(vl_previsto_saldo,0)   as 'SaldoDiario',
  cast(0.00 as float)           as vl_aberto_documento_receber, 
  cast(0.00 as float)           as vl_pago_documento_receber,   
  cast(0.00 as float)           as vl_aberto_documento_pagar,   
  cast(0.00 as float)           as vl_pago_documento_pagar,
  isnull(vl_previsto_saldo,0)   as 'SaldoAcumulado'
...
select 
 isnull((select sg_semana 
         from semana with (nolock) 
         where cd_semana = DATEPART(dw , Vencimento)),'') as dia_semana,
 * 
from
  #Tab_Aux 
order by 
