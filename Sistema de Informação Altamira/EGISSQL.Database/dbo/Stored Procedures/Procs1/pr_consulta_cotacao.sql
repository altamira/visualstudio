...
-------------------------------------------------------------------------------
  begin
    select cc.*,
           ...
           d.nm_departamento,
           cond_pagto.nm_condicao_pagamento
    from cotacao cc
    ...
    left outer join condicao_pagamento cond_pagto
    on cc.cd_condicao_pagamento = cond_pagto.cd_condicao_pagamento
    where (cc.cd_cotacao = @cd_cotacao_compra) or
          (cc.dt_cotacao between @dt_inicial and @dt_final ) and
          ((@ic_status = 0) or
           ((@ic_status <> 0) and (dt_fechamento_cotacao is not null))  ) and
          (@cd_cotacao_compra = 0)
