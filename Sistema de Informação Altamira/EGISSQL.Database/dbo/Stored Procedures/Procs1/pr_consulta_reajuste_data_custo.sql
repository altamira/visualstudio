...
-------------------------------------------------------------------------------
--pr_consulta_reajuste_data_custo
-------------------------------------------------------------------------------
...
-- 20.11.2008 - Preço de Custo - Carlos Fernandes
--------------------------------------------------------------------------------------------------
create procedure pr_consulta_reajuste_data_custo
@dt_base          datetime,
...
--------------------------------------------------------------------------------------------------
  select
    ...
    pc.vl_custo_produto        as ValorAtual,
    ph.vl_historico_produto    as ValorAnterior,
    case when pc.vl_custo_produto>0 then 
        cast(((pc.vl_custo_produto / ph.vl_historico_produto) - 1) * 100 as money) else 0 end as Variacao,
    ph.cd_usuario,
  ...
  select
    ...
    um.sg_unidade_medida               as Unidade,
    s.vl_custo_servico                 as ValorAtual,
    sh.vl_servico_historico            as ValorAnterior,
    case when s.vl_custo_servico>0 then 
        cast(((s.vl_custo_servico / sh.vl_servico_historico) - 1) * 100 as money) else 0 end as Variacao,
    sh.cd_usuario,
    u.nm_fantasia_usuario              as Usuario,
    tr.nm_tipo_reajuste                as TipoReajuste,
    ''                                 as MotivoReajuste,
