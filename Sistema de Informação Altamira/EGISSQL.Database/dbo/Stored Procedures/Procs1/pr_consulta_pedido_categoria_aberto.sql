...
-- 02.04.2008 - Ajuste do Pedido de Venda Produto/Serviço - Carlos Fernandes
-- 09.03.2010 - Observações da Fábrica - Carlos Fernandes
-- 10.03.2010 - Desenho do Cadastro do Produto - Carlos Fernandes
-- 12.04.2010 - Desenho do Cadastro do Item do Pedido de Venda - Carlos Fernandes
---------------------------------------------------------------------------------------------------------------
...
Begin
  ...
  select 
    ...
    (pvi.qt_saldo_pedido_venda * pvi.vl_unitario_item_pedido) as vl_total_saldo_aberto,
     ...
     pvi.nm_observacao_fabrica1   as 'Observacao1',
     pvi.nm_observacao_fabrica2   as 'Observacao2',

     case when isnull(pvi.cd_desenho_item_pedido,'')<>'' and
        pvi.cd_desenho_item_pedido <> p.cd_desenho_produto 
     then
     ...
     case when isnull(pvi.cd_rev_des_item_pedido,'')<>'' and
        pvi.cd_rev_des_item_pedido <> p.cd_rev_desenho_produto
     then
...
--select * from pedido_venda_item
  ...
  left outer join Produto_Cliente pc   with(nolock) on  pc.cd_cliente =  pv.cd_cliente and pc.cd_produto = pvi.cd_produto
...
--                                 end ) and --Limita os pedidos nacionais até 80, devido a uma regra de negócio aplicada     
     ...
     isnull(pvi.cd_produto_servico,0) = 0           -- Não Mostra os Itens de Serviço relacionado a Produto 
...
--       (nsi.qt_item_nota_saida * nsi.vl_unitario_item_nota)) as vl_total_saldo_faturado,
   ...
   -------
   from Pedido_Venda_Item pvi          with(nolock) 
        left join Pedido_Venda pv      with(nolock) on (pvi.cd_pedido_venda = pv.cd_pedido_venda)
        left join Nota_Saida_Item nsi  with(nolock) on (pvi.cd_pedido_venda = nsi.cd_pedido_venda and pvi.cd_item_pedido_venda = nsi.cd_item_pedido_venda) 
        left join Nota_Saida ns        with(nolock) on (nsi.cd_nota_saida = ns.cd_nota_saida) 
        left join Operacao_Fiscal op   with(nolock) on (ns.cd_operacao_fiscal = op.cd_operacao_fiscal) 
        left join Categoria_produto cp with(nolock) on (pvi.cd_categoria_produto = cp.cd_categoria_produto)
        left join Grupo_Categoria gc   with(nolock) on (cp.cd_grupo_categoria = gc.cd_grupo_categoria)
...
--                                 end ) and --Limita os pedidos nacionais até 80, devido a uma regra de negócio aplicada     
    ...
    select 
       ...
       - 
       pvi.dt_entrega_vendas_pedido as int )                  as qt_atraso,
      ...
      (isnull(a.qt_saldo_pedido_venda,0) + 
       ...
       pvi.nm_observacao_fabrica1   as 'Observacao1',
       pvi.nm_observacao_fabrica2   as 'Observacao2',
--       pro.cd_desenho_produto,
--       pro.cd_rev_desenho_produto,

      case when isnull(pvi.cd_desenho_item_pedido,'')<>'' and
         pvi.cd_desenho_item_pedido <> pro.cd_desenho_produto 
      then
         pvi.cd_desenho_item_pedido
      else
        pro.cd_desenho_produto
      ...
      case when isnull(pvi.cd_rev_des_item_pedido,'')<>'' and
         pvi.cd_rev_des_item_pedido <> pro.cd_rev_desenho_produto
      then
         pvi.cd_rev_des_item_pedido
      else
        pro.cd_rev_desenho_produto
...
--    (i.qt_item_pedido_venda * i.vl_unitario_item_pedido) AS 'vl_total_pedido_venda', --Valor
    ...
    i.nm_observacao_fabrica1   as 'Observacao1',
    i.nm_observacao_fabrica2   as 'Observacao2',
--    pro.cd_desenho_produto,
--    pro.cd_rev_desenho_produto,
      case when isnull(i.cd_desenho_item_pedido,'')<>'' and
         i.cd_desenho_item_pedido <> pro.cd_desenho_produto 
      then
         i.cd_desenho_item_pedido
      else
        pro.cd_desenho_produto
      end                          as cd_desenho_produto,

      case when isnull(i.cd_rev_des_item_pedido,'')<>'' and
         i.cd_rev_des_item_pedido <> pro.cd_rev_desenho_produto
      then
         i.cd_rev_des_item_pedido
      else
        pro.cd_rev_desenho_produto
      end                          as cd_rev_desenho_produto,

    i.cd_pdcompra_item_pedido 
  ...
  from 
    ...
    left outer join Produto pro          on pro.cd_produto = i.cd_produto
  ...
  where
    ...
    cp.cd_mascara_categoria like @cd_categoria+'%'      and 
    isnull(cp.ic_vendas_categoria,'S')='S'              and
    isnull(i.cd_produto_servico,0)    = 0            
