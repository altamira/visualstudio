...
--sp_helptext pr_analise_movimento_caixa_por_tipo_pedido

CREATE PROCEDURE pr_analise_movimento_caixa_por_tipo_pedido
...
SELECT     
           ...
           case when sum(isnull(dr.vl_saldo_documento,0))>0 and 
                     dbo.fn_dia_util(( dr.dt_vencimento_documento + IsNull(max(p.qt_credito_efetivo),0) ),'S','U') < getdate() - 2
                then
                  sum(isnull(dr.vl_saldo_documento,0))
                else           
                  cast( 0 as float)
...
(
 ...
 + 
  ...
  cpp.qt_dia_cond_parcela_pgto )>getdate() and
...
where
  ...
  isnull(pci.dt_entrega_item_ped_compr,pc.dt_pedido_compra) + cpp.qt_dia_cond_parcela_pgto > getdate() and
...
where
  --pc.dt_pedido_compra between @dt_inicial and @dt_final and
  isnull(pii.dt_prev_embarque_ped_imp,p.dt_prev_emb_ped_imp) + cpp.qt_dia_cond_parcela_pgto > getdate() and
