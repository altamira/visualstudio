...
-- 13.09.2006 - Revisão e colocação da Quantidade de produção do produto - Carlos Fernandes
-- 25.11.2008 - Ajustes Diversos - Carlos Fernandes
-- 10.02.2009 - Categoria de Produto / Grupo de Produto - Carlos Fernandes
-- 10.03.2009 - Ajuste de performance - with no lock - Carlos Fernandes
-- 27.04.2009 - Se a empresa opera com Alocação, o Disponível será outro cálculo - Carlos Fernandes
-- 08.10.2009 - Ajuste na Qtd. de Importação - Carlos Fernandes/Telma
-- 25.05.2010 - Verificação da Fase do Produto - Carlos Fernandes
-----------------------------------------------------------------------------------------------------------------------
...
begin
	...
	declare 
	  ...
	  @ic_numero_de_NAO_finalidade  varchar(30),
          ...
          @ic_previsao_evolucao_consumo char(1),
          @ic_alocacao_estoque_reserva  char(1)
  ...
  select
    @ic_mostrar_mes_atual         = isnull(ic_mostrar_mes_atual        ,'N'),
    @cd_fase_produto_entreposto   = IsNull(cd_fase_produto_entreposto  , @cd_fase_produto), --Define a fase do entreposto - Fabio
    @ic_previsao_evolucao_consumo = IsNull(ic_previsao_evolucao_consumo,'C'),
  ...
  Select ------------------------------------------------------------------- SELECT
    ...
    --Dt_Prev_Entrada1
    case @ic_previsao_evolucao_consumo
    ...
    case when @ic_alocacao_estoque_reserva='N'
    then
      ...
      isnull(( select sum(qt_estoque) 
        from
          estoque_pedido_venda with (nolock) 
        where
          cd_produto = p.cd_produto ),0)
...
--    isnull(ps.qt_importacao_produto,0) +
    ...
    --isnull(ps.qt_producao_produto,0)   as 'Qtd_Producao',
    isnull(( select sum(qt_planejada_processo)
      from
        vw_processo_producao_produto vwp
      where
        vwp.cd_produto = p.cd_produto ),0)  as 'Qtd_Producao',
    ...
    isNull(ps.qt_saldo_atual_produto,0.00)*
    dbo.fn_vl_custo_unitario_produto( p.cd_produto ) as 'Vlr_Custo_Total',
    ...
    cast(0 as float)       as 'qtd_est_producao',
    ...
    -- Data da última Venda
     ...
     where
       ...
       ns.cd_status_nota <> 7               --nota cancelda
     order by
...
--      IsNull((select Sum( isnull(pii.qt_saldo_item_ped_imp,0) )
...
--                   inner join tipo_pedido tp      on tp.cd_tipo_pedido      = pI.cd_tipo_pedido
...
--              group by pii.cd_produto),0)                             as 'Qtd_Pedido_Imp',
    isnull( (select sum( vwe.Quantidade )
            from
              vw_previsao_entrada vwe
            where
              vwe.CodigoProduto = p.cd_produto
            group by
              vwe.CodigoProduto ),0)                     as 'Qtd_Pedido_Imp',
     ...
     --Programação de Contrato / Fornecimento

     ( select sum(qt_contrato_fornecimento) 
       from
         vw_programacao_contrato_produto vwp with (nolock) 
       where
         vwp.cd_produto = p.cd_produto ) as 'Qtd_Contrato',
    ...
    -- Data da última entrada
     (Select top 1
      ...
      from
        ...
        inner join nota_entrada_item nei with (nolock) on nei.cd_nota_entrada   = ne.cd_nota_entrada
        inner join Operacao_Fiscal op    with (nolock) on ne.cd_operacao_fiscal = op.cd_operacao_fiscal 
     where
       op.cd_tipo_movimento_estoque = 1               and
       isnull(op.ic_comercial_operacao,'N')     = 'S' and
       nei.cd_produto = p.cd_produto
     order by
       ne.dt_nota_entrada desc) as 'Ultima_Entrada',

     --Perda de Produto
     ( select sum(qt_item_consulta) 
       from
         vw_perda_produto vwp with (nolock) 
       where
         vwp.cd_produto = p.cd_produto and
         vwp.dt_perda_consulta_itens between @dt_inicial and getdate() ) as 'Qtd_Perda',
     ...
     where
       ...
       nei.cd_produto = p.cd_produto
     order by
       ne.dt_nota_entrada desc) as 'Qtd_Ultima_Entrada',

     cast(0.00 as float)                  as 'MEDIA_CONSUMO',
     cast(0.00 as float)                  as 'Duracao',
     --cast(0.00 as float)                  as 'Alocado',
     ( select sum(qt_atendimento) 
       from 
         atendimento_pedido_venda apv with (nolock) 
       where
         p.cd_produto = apv.cd_produto )  as 'Alocado', 
     pimp.cd_part_number_produto          as 'cd_part_number_produto',
     isnull(pimp.vl_produto_importacao,0) as 'vl_produto_importacao',
     cast(0.00 as float)                  as 'qt_previsao_venda',
     cast(0.00 as float)                  as 'qt_proposta',
     cast(0.00 as float)                  as 'qt_faturada',
...
--select * from produto_importacao
  ...
  From ------------------------------------------------------------------------- FROM
    ...
    inner join Produto p                    with (nolock) on p.cd_produto         = #Selecao_Inicial.cd_chave_produto
    ...
    left outer join Produto_Saldo ps        with (nolock) on ps.cd_produto        = p.cd_produto and
                                                             ps.cd_fase_produto   = case when isnull(@cd_fase_produto,0)>0 then
                                                                                      @cd_fase_produto
                                                                                    else
...
--@cd_fase_produto
    ...
    left outer join Produto_Saldo pse       with (nolock) on pse.cd_produto       = p.cd_produto and
                                                             pse.cd_fase_produto  = @cd_fase_produto_entreposto

    left outer join  Pais                   with (nolock) on Pais.cd_pais         = p.cd_origem_produto
    ...
    left outer join Fase_Produto fp         with (nolock) on fp.cd_fase_produto      = case when isnull(@cd_fase_produto,0)>0 then
                                                                                         @cd_fase_produto
                                                                                       else
                                                                                         case when isnull(p.cd_fase_produto_baixa,0)<>0 then
                                                                                            p.cd_fase_produto_baixa
...
'        ( select
  ...
  begin
    ...
    '  set X_Anual = X_Anual + mov.Qtde, ' + 
         ...
         ' MEDIA_CONSUMO = MEDIA_CONSUMO + mov.QtdeTrim, ' + 
...
--Carlos 20.01.2005
  ...
  set @cSql = 
   ' declare @Consumo int, ' + 
         ' @Saldo float, ' + 
         ' @QtdComprar float, ' + 
         ' @Trimestral float, @MediaConsumo float '  
  ...
  set @cSql = @cSql + 
    ...
    ' Media_Consumo = cast( MEDIA_CONSUMO / case when @Consumo>0 then @Consumo else 1 end as int ),' +
    ' Duracao       = @Saldo / (case when MEDIA_CONSUMO>0 then MEDIA_CONSUMO ELSE 1 end / case when @Consumo>0 then @Consumo else 1 end ), '+
     -- Saldo em Estoque
