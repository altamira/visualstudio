...
-------------------------------------------------------------------------------
--sp_helptext pr_calculo_comissao_faturamento_pagamento
-------------------------------------------------------------------------------
--pr_calculo_comissao_faturamento_pagamento
-------------------------------------------------------------------------------
...
-- 22.01.2010 - Quando o (%) for zero a comissão deverá ser zero - Carlos Fernandes
-- 10.05.2010 - Vendor/Crédito Rural - Carlos Fernandes
-- 10.12.2010 - Frete x Dedução Comissão - Carlos Fernandes
-- 14.12.2010 - Rateio do Frete por Item - Carlos Fernandes

----------------------------------------------------------------------------------
create procedure pr_calculo_comissao_faturamento_pagamento
@cd_vendedor int      = 0,
@dt_inicial  datetime = '',
@dt_final    datetime = '',
@cd_usuario  int      = 0
...
declare @vl_frete_comissao         float
...
where
 ...
 ( vw.cd_status_nota        = 5 or --Nota Fechada
   vw.cd_status_nota        = 3  ) --Nota com Dev. Parcial

group by
  vw.cd_nota_saida

--select * from vw_faturamento where cd_nota_saida = 14907
...
begin
  ...
  begin
    ...
    select
      ...
      isnull(cp.pc_comissao_cat_produto,0)              as pc_comissao_cat_produto,
      ...
      vl_base_comissao = --( vw.vl_unitario_item_total-isnull(nsi.vl_icms_item,0)-isnull(nsi.vl_ipi,0)) 
                         ((vw.vl_unitario_item_total - isnull(vwf.vl_item_frete_comissao,0) - isnull(nsi.vl_icms_item,0) - isnull(nsi.vl_ipi,0))
...
--                            1
     ...
     *  (cp.pc_comissao_cat_produto/100)                                   as vl_comissao_vendedor,
     ...
     - 
     (vw.vl_unitario_item_total - isnull(vwf.vl_item_frete_comissao,0) - isnull(nsi.vl_icms_item,0) - isnull(nsi.vl_ipi,0) )
     * case when isnull(pf.pc_reducao_piscofins,1)<>0 then pf.pc_reducao_piscofins else 1 end )                                 as vl_reducao,

     isnull(vp.pc_red_comissao_vendedor,0)                               as pc_red_comissao_vendedor,

     isnull(cf.vl_frete_nota_comissao,0)                                 as vl_frete_comissao
   ...
   from
     ...
     left outer join comissao_frete cf     with (nolock) on cf.cd_identificacao_nota_saida = vw.cd_identificacao_nota_saida
     --Interno
     ...
     left outer join vw_rateio_frete_comissao vwf with (nolock) on  vwf.cd_nota_saida       = vw.cd_nota_saida and
                                                                    vwf.cd_item_nota_saida  = vw.cd_item_nota_saida
...
--     vw.cd_status_nota        = 5 --Nota Fechada
  ...
  -- select * from #RateioProduto where cd_vendedor=308
 
   select
     @vl_base_comissao_nota     = sum( isnull(vl_base_comissao,0) ),
     @vl_frete_comissao         = max(case when isnull(vl_frete_comissao,0)>0 then vl_frete_comissao else 0.00 end ),
     ...
     @vl_comissao_nota          = sum( case when isnull(pc_emissao_nota_comissao,0)>0 then
                                       isnull(vl_comissao_vendedor,0)
                                         ...
                                         case when isnull(pc_baixa_comissao,0)>0 then
                                             isnull(vl_comissao_vendedor,0)
                                         else
                                            0.00
                                         end
   ...
   select 
     @pc_comissao_nota = 
         sum(case when isnull(pc_emissao_nota_comissao,0)>0
             then
               round((isnull(vl_comissao_vendedor,0) / case when isnull(@vl_base_comissao_nota,0)>0 then @vl_base_comissao_nota else 1 end * 100),2) 
             else
               case when isnull(pc_baixa_comissao,0)>0
               then
                 round((isnull(vl_comissao_vendedor,0) / case when isnull(@vl_base_comissao_nota,0)>0 then @vl_base_comissao_nota else 1 end * 100),2) 
                else
                  0.00
   ...
   if @pc_comissao_nota > 10 
      set @pc_comissao_nota = 10
  ...
  if @qt_parcela_comissao_nota is null or @qt_parcela_comissao_nota = 0
   ...
   if not exists( select top 1 cd_nota_saida from nota_saida_comissao with (nolock) 
                  where
                    cd_nota_saida = @cd_nota_saida )
...
--Documentos Baixados no Período
...
--       (((nsc.vl_icms_comissao_nota    /nsc.qt_parcela_comissao_nota) * ( (drp.vl_pagamento_documento/d.vl_documento_receber)))) as vl_icms,
--       (((nsc.vl_ipi_comissao_nota     /nsc.qt_parcela_comissao_nota) * ( (drp.vl_pagamento_documento/d.vl_documento_receber)))) as vl_ipi,
--       (((nsc.vl_red_piscofins_comissao/nsc.qt_parcela_comissao_nota) * ( (drp.vl_pagamento_documento/d.vl_documento_receber)))) as vl_redpiscofins,
...
--       isnull(drp.vl_abatimento_documento,0)
      ...
      - ( ((nsc.vl_icms_comissao_nota    /nsc.qt_parcela_comissao_nota) * ( (drp.vl_pagamento_documento/d.vl_documento_receber))))
      - ( ((nsc.vl_ipi_comissao_nota     /nsc.qt_parcela_comissao_nota) * ( (drp.vl_pagamento_documento/d.vl_documento_receber))))
      - ( ((nsc.vl_red_piscofins_comissao/nsc.qt_parcela_comissao_nota) * ( (drp.vl_pagamento_documento/d.vl_documento_receber))))
      )
      *
      case when isnull(vp.pc_baixa_comissao,0)>0 then
...
--       isnull(drp.vl_abatimento_documento,0) 
      ...
      *
      --Verifica se Paga Comissão pelo Tipo de Liquidação
      case when isnull(tl.ic_comissao_tipo_liquidacao,'S')='N' 
      then
        0
      else
        1
    ...
    from
      ...
      left outer join Vendedor v              with (nolock) on (v.cd_vendedor            = 
                                                                case when isnull(ns.cd_nota_saida,0)>0 and
                                                                  ns.cd_vendedor<>d.cd_vendedor then ns.cd_vendedor
                                                                else
                                                                  case when isnull(d.cd_vendedor,0)>0 then
                                                                     d.cd_vendedor else c.cd_vendedor
                                                                  end
                                                                end)
      left outer join Vendedor_Parametro vp   with (nolock) on vp.cd_vendedor = v.cd_vendedor
...
--      and ns.cd_status_nota = 5
      and
      ( ns.cd_status_nota        = 5 or --Nota Fechada
        ns.cd_status_nota        = 3  ) --Nota com Dev. Parcial
...
--   end)                                  as 'cd_nota_saida',
  ...
  max(isnull(cf.vl_frete_nota_comissao,0))   as vl_frete_comissao
...
from
  ...
  left outer join comissao_frete cf on cf.cd_identificacao_nota_saida = vw.cd_identificacao_nota_saida
