...
-------------------------------------------------------------------------------
--sp_helptext pr_consulta_producao_apontamento_producao
-------------------------------------------------------------------------------
...
--                   Por data de Produção conforme o lançamento Manual
--Data             : 16.08.2007
...
-- 14.03.2009 - Complemento dos Campos - Carlos Fernandes
-- 25.05.2009 - Ajuste da Quantidade do Refugo - Carlos Fernandes
-- 05.12.2009 - Preço do Custo de Produção - Carlos Fernandes
-----------------------------------------------------------------------------------------
create procedure pr_consulta_producao_apontamento_producao
@dt_inicial datetime = '',
@dt_final   datetime = ''
as
...
--   isnull(ppa.ic_operacao_concluida,'N')    ='S'             and
...
select
  ...
  case when isnull(pc.vl_custo_producao_produto,0)>0 then pc.vl_custo_producao_produto else pc.vl_custo_produto end as CustoProducao,
...
--select * from processo_producao_composicao where cd_processo = 1777
--select * from Processo_Producao_Apontamento ppa where cd_processo = 1777
