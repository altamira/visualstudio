...
---------------------------------------------------
-- pr_analise_custo_peps
---------------------------------------------------
--GBS - Global Business Solution	       2002
--Stored Procedure: Microsoft SQL Server       2000
--Autor(es)     : Elias Pereira da Silva
--Banco de Dados: EgisSQL
--Objetivo      : Lista Consulta de Análise do Cálculo do
--                Custo PEPS
--Data          : 28/07/2004
--Atualizado    : 02/08/2004 - Diversos Acertos
---------------------------------------------------
create procedure pr_analise_custo_peps
...
as
  ...
  select 
    ...
    cast(isnull(pf.vl_custo_peps_fechamento, isnull(pf.vl_custo_prod_fechamento,0)) / 
    ...
    cast(isnull(pf.vl_custo_peps_fechamento,isnull(pf.vl_custo_prod_fechamento,0)) as decimal(25,2)) as vl_total_saldo
  into #SI
...
--        isnull(pc.ic_peps_produto,'S') = 'S' and
  ...
  select
    ...
    case when tme.ic_mov_tipo_movimento = 'S' then 
      cast(isnull(me.vl_custo_contabil_produto,0) as decimal(25,2)) else 0 end as vl_preco_saida,
    case when tme.ic_mov_tipo_movimento = 'S' then 
    cast(isnull(me.vl_custo_contabil_produto,0) * isnull(me.qt_movimento_estoque,0) as decimal(25,2)) else 0 end as vl_total_saida,
...
--      pf.dt_produto_fechamento between @dt_inicial and @dt_final
  ...
  begin
    ...
    -- LISTA O SALDO ANTERIOR
    select
      pf.cd_produto,
      cast(pf.qt_atual_prod_fechamento as decimal(25,4)) as qt_saldo_anterior,
      cast(isnull(pf.vl_custo_peps_fechamento,0) / 
           case when (isnull(pf.qt_atual_prod_fechamento,0) = 0) then 1 
                else pf.qt_atual_prod_fechamento end as decimal(25,2)) as vl_preco_saldo_anterior,
      cast(isnull(pf.vl_custo_peps_fechamento,0) as decimal(25,2)) as vl_total_saldo_anterior
    into #SA
    from Produto_Fechamento pf
    where pf.cd_fase_produto = @cd_fase_produto and
          pf.dt_produto_fechamento = (@dt_inicial - 1) and
          pf.cd_produto in (select distinct cd_produto from #M)


    -- LISTA O SALDO FINAL
    select
      pf.cd_produto,
      cast(pf.qt_atual_prod_fechamento as decimal(25,4)) as qt_saldo_final,
      cast(isnull(pf.vl_custo_peps_fechamento,0) / 
           case when (isnull(pf.qt_atual_prod_fechamento,0) = 0) then 1 
                else pf.qt_atual_prod_fechamento end as decimal(25,2)) as vl_preco_saldo_final,
      cast(isnull(pf.vl_custo_peps_fechamento,0) as decimal(25,2)) as vl_total_saldo_final
    into #SF
    from Produto_Fechamento pf
    where pf.cd_fase_produto = @cd_fase_produto and
    ...
    select
      ...
      m.ds_produto,
      m.cd_produto,
      sa.qt_saldo_anterior,
      sa.vl_preco_saldo_anterior,
      sa.vl_total_saldo_anterior,
      ...
      sf.qt_saldo_final,
      sf.vl_preco_saldo_final,
      sf.vl_total_saldo_final
    from #M m
    left outer join #SA sa
    on m.cd_produto = sa.cd_produto
    left outer join #SF sf
    on m.cd_produto = sf.cd_produto
--    where m.cd_entrada_saida in ('S','E')
    group by m.nm_grupo_produto, m.nm_produto, m.ds_produto, m.cd_produto,      
             sa.qt_saldo_anterior, sa.vl_preco_saldo_anterior, sa.vl_total_saldo_anterior,
             sf.qt_saldo_final, sf.vl_preco_saldo_final, sf.vl_total_saldo_final
