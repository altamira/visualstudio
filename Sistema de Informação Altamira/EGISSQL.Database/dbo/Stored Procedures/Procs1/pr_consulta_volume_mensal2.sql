...
CREATE PROCEDURE pr_consulta_volume_mensal2
...
begin
    ...
    where
     ...
     ((vw.cd_categoria_produto = @cd_categoria_produto) or 
      ...
      (( @cd_vendedor = 0 ) or ( @cd_vendedor <> 0  and vw.cd_vendedor = @cd_vendedor)) 
    group by 
...
SELECT 
  ...
  IsNull((Select sum(Pedidos) from #VendaAnualMes 
          where 
            ...
            Vendedor = a.Vendedor and
            Grupo = a.Grupo and
            Cliente = a.Cliente and
            CodVendedor = a.CodVendedor and
            Estado = a.Estado
            and NumeroMes = 1),0),
  IsNull((Select sum(Pedidos) from #VendaAnualMes 
          where 
