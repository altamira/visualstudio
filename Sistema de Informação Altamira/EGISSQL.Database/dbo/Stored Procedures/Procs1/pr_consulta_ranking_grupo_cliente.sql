...
create procedure pr_consulta_ranking_grupo_cliente
...
--Autor       : Daniel Duela
--Objetivo    : Consulta do Ranking de Grupo de Clientes
--Data        : 12/01/2004
...
-- 12.05.2010 - Devolução - Carlos Fernandes
--------------------------------------------------------------------------------------
@ic_parametro    int,
@dt_inicial      datetime,
@dt_final        datetime,
...
declare @qt_total_grupo_cliente int
declare @vl_total_grupo_cliente float
...
begin
  ...
  select 
    cg.cd_cliente_grupo,
    cg.nm_cliente_grupo as 'Grupo_Cliente',
  ...
  left outer join Cliente cli on
    vw.cd_cliente=cli.cd_cliente 
  left outer join Cliente_Grupo cg on
    cli.cd_cliente_grupo=cg.cd_cliente_grupo
  ...
  -- Total de Cliente
  set @qt_total_grupo_cliente = @@rowcount

  -- Total de Vendas Geral
  set @vl_total_grupo_cliente = 0
  select @vl_total_grupo_cliente = @vl_total_grupo_cliente + compra
  from
    #VendaClienteAux1
  ...
  select 
    ...
    MargemContribuicao,
    BNS,
    cast(((Compra / @vl_total_grupo_cliente ) * 100) as decimal (25,2)) as 'Perc',
    UltimaCompra,
    pedidos,
...
--  	(vw.dt_nota_saida < @dt_inicial) and
  ...
  select 
    IDENTITY(int, 1,1) as 'Posicao',
    b.cd_cliente_grupo,      
    b.nm_cliente_grupo as 'Grupo_Cliente', 
    ...
    sum(a.BNS) as 'BNS',
    sum(cast(((a.Venda / @vl_total_grupo_cliente ) * 100) as decimal (15,2))) as 'Perc',
  ...
  from 
    ...
    Cliente c
      ...
      left outer join
    Cliente_Grupo b 
      on c.cd_cliente_grupo = b.cd_cliente_grupo
  group by
    b.cd_cliente_grupo,      
    b.nm_cliente_grupo
