...
declare @qt_km_aviso_troca_oleo float
        ...
        select
          ...
          case when
            isnull(mv.qt_km_validade_veiculo,0) - (isnull(rv.qt_km_final,0) - isnull(mv.qt_km_inicial_veiculo,0)) < @qt_km_aviso_troca_oleo
          then
            'Sim'
          else
            'Não' end                                      as Troca,
...
--          (Select top 1 mvaux.dt_movimento_veiculo from Movimento_Veiculo mvaux where  mvaux.cd_veiculo = v.cd_veiculo and mvaux.dt_movimento_veiculo > mv.dt_movimento_veiculo order by mvaux.dt_movimento_veiculo) as dt_proximotroca
