...
CREATE PROCEDURE pr_venda_resumo_produto_cliente
---------------------------------------------------
...
as
  ...
  where
     ...
     isnull(cd_cliente,0) = case isnull(@cd_cliente,0) when 0 then isnull(cd_cliente,0) else isnull(@cd_cliente,0) end         
  Group by cd_cliente,     a.cd_produto, 
    (case IsNull(a.cd_produto,0)
      when 0 then
        a.nm_categoria_produto
      else
  ...
  from 
    ...
    Cliente d  on a.cd_cliente = d.cd_cliente left join
