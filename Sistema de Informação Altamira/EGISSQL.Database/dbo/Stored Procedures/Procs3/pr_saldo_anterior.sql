...
/****** Object:  Stored Procedure dbo.pr_saldo_anterior    Script Date: 13/12/2002 15:08:42 ******/
----------------------------------------------------------------------------------
--c
-----------------------------------------------------------------------------------
...
--Objetivo                : Cálculo do Saldo Anterior das Contas
--Data                    : 06.07.2001
...
--                        : 30/12/2004 - Acerto do Carregamento da Variável da Data da Conta - ELIAS
--                        : 02.02.2006 - Correção da busca da máscara da conta - Carlos Fernandes
----------------------------------------------------------------------------------------------------
CREATE  procedure pr_saldo_anterior
@ic_parametro      int,
@cd_empresa        int,
@cd_exercicio      int,
...
begin
  ...
  set @dt_saldo_conta = (select 
                           max(dt_saldo_conta) 
                         from
                           Saldo_conta
                         where cd_empresa = @cd_empresa and
                           dt_saldo_conta between '01/01/1900' and @dt_base_saldo)

  if (isnull(@dt_saldo_conta,0)=0) 
    set @dt_saldo_conta = '01/01/1900'
  ...
  -----------------------------------------------------------------------------
    begin
      ...
      into 
        #SaldoConta
      from
        Plano_conta p left outer join Saldo_conta s on s.cd_conta = p.cd_conta 
                                                   and s.dt_saldo_conta = @dt_saldo_conta
      where
