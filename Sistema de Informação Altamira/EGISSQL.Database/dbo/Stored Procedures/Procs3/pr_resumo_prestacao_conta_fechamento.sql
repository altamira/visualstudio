...
--USE EGISSQL
-------------------------------------------------------------------------------
--sp_helptext pr_resumo_prestacao_conta_fechamento
-------------------------------------------------------------------------------
...
-- 04.07.2008 - Centro Custo/Data do Fechamento - Carlos Fernandes
-- 11.08.2008 - Separação do tipo fornecedor - Carlos Fernandes
------------------------------------------------------------------------------
create procedure pr_resumo_prestacao_conta_fechamento
@ic_parametro        int         = 0,
@dt_inicial          datetime    = '',
@dt_final            datetime    = '',
...
-------------------------------------------------------------------------------
--Inclusão dos Funcionários lançados na prestação de contas de Fornecedores
-------------------------------------------------------------------------------
  ...
  isnull(tv.nm_tipo_viagem,'')                 as TipoViagem,
    ...
    isnull(( select
      ...
      from
        Solicitacao_Adiantamento sa
      where
        sa.cd_prestacao = pc.cd_prestacao and
        sa.cd_funcionario = pcc.cd_funcionario_composicao),0)   as TotalAdiantamento,

    --Total de Despesas

     pcc.vl_total_despesa                        as TotalDespesa, 
    ...
    isnull((select
       isnull(sum( isnull(vl_total_despesa,0) ),0)
     from
       prestacao_conta_composicao xpcc with (nolock) 
       left join tipo_despesa td      with (nolock) on td.cd_tipo_despesa = xpcc.cd_tipo_despesa  
     where
       xpcc.cd_prestacao = pc.cd_prestacao and
       xpcc.cd_funcionario_composicao = pcc.cd_funcionario_composicao and
       isnull(td.ic_reembolsavel_despesa,'N') = 'N' ),0) as TotalNaoReembolsavel,
   ...
   from
     Prestacao_Conta pc                             with (nolock) 
     left outer join Prestacao_Conta_Composicao pcc with (nolock) on pcc.cd_prestacao        = pc.cd_prestacao
     left outer join Funcionario fo                 with (nolock) on fo.cd_funcionario       = pc.cd_funcionario
     left outer join Funcionario f                  with (nolock) on f.cd_funcionario        = pcc.cd_funcionario_composicao 
...
begin
  ...
  select
    ...
    pc.dt_fechamento_prestacao                   as DataPrestacao,
...
--         pcc.cd_prestacao = pc.cd_prestacao ),0)  as TotalDespesa,
   ...
   from
     ...
     left outer join Tipo_Viagem       tv        with (nolock) on tv.cd_tipo_viagem       = rv.cd_tipo_viagem
     left outer join Tipo_Cartao_Credito tc      with (nolock) on tc.cd_cartao_credito    = pc.cd_cartao_credito
     left outer join Funcionario fc              with (nolock) on fc.cd_funcionario       = pc.cd_funcionario
