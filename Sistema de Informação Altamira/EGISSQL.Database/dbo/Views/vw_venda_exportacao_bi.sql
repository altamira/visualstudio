...
CREATE VIEW vw_venda_exportacao_bi
AS 
  select
    pv.cd_pedido_venda,
    pv.dt_pedido_venda,
  ...
  from
    ...
    Tipo_Mercado        tm  on tm.cd_tipo_mercado       = c.cd_tipo_mercado       left outer join
    Pais                p   on p.cd_pais                = c.cd_pais
--select * from tipo_mercado
  where
    isnull(tm.ic_exportacao_tipo_mercado,'N')='S'                                                        and
    ...
    pvi.qt_item_pedido_venda * pvi.vl_unitario_item_pedido 	           >  0
