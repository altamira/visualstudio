...
-- 20.08.2009 - Nota de Importação - Carlos Fernandes
-- 24.09.2009 - Importação - Carlos Fernandes
-- 18.03.2010 - Subst. Tributária - Carlos Fernandes
-----------------------------------------------------------------------------------------
-- 
--
create view vw_livro_registro_saida_reg50_entrada
as
...
--      nsri.sg_estado                      as 'UF',
      ...
      case when dbo.fn_status_cancelada( ns.cd_status_nota ) = 'S' then 0.00 else nsri.VlrContabil end  as 'VlrContabil',
      case when dbo.fn_status_cancelada( ns.cd_status_nota ) = 'S' then 0.00 else nsri.BCICMS      end  as 'BCICMS',
      case when dbo.fn_status_cancelada( ns.cd_status_nota ) = 'S' then 0.00 else nsri.AliqICMS    end  as 'AliqICMS',
      case when dbo.fn_status_cancelada( ns.cd_status_nota ) = 'S' then 0.00 else nsri.ICMS        end  as 'ICMS',
      case when dbo.fn_status_cancelada( ns.cd_status_nota ) = 'S' then 0.00 else nsri.ICMSIsento  end  as 'ICMSIsento',
      case when dbo.fn_status_cancelada( ns.cd_status_nota ) = 'S' then 0.00 else nsri.ICMSOutras  end  as 'ICMSOutras',
      case when dbo.fn_status_cancelada( ns.cd_status_nota ) = 'S' then 0.00 else nsri.BCICMSST    end  as 'BCICMSST',
      case when dbo.fn_status_cancelada( ns.cd_status_nota ) = 'S' then 0.00 else nsri.ICMSST      end  as 'ICMSST'
    ...
    ( select distinct
        ...
        sum(isnull(case when isnull(o.ic_importacao_op_fiscal,'N')='S'
        then
          r.vl_base_icms_item_nota
        else
          r.vl_contabil_item_nota
        end,0))                                                 as 'VlrContabil',
