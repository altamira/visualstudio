...
CREATE VIEW [dbo].[Acompanhamento do Orçamento]
AS
...
SELECT  
	...
	(SELECT TOP 1 Historico.[Última Atualização] FROM dbo.[Histórico da Situação do Orçamento] Historico WHERE Historico.[Número do Orçamento] = Informações.[Número do Orçamento] ORDER BY [Última Atualização] DESC) AS [Última Alteração],
	...
	(SELECT TOP 1 Historico.[Observações] FROM dbo.[Histórico da Situação do Orçamento] Historico WHERE Historico.[Número do Orçamento] = Informações.[Número do Orçamento] ORDER BY [Última Atualização] DESC) AS [Observações]
FROM 
...
WHERE ISNUMERIC(WBCCAD.Orçamento.Número) = 1 /*AND WBCCAD.Orçamento.[Código do Representante] = '121'*/
