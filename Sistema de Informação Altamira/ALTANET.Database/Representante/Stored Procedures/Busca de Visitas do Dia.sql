...
CREATE PROCEDURE [Representante].[Busca de Visitas do Dia]
...
BEGIN
	...
	WHERE 
		...
		AND CAST([Visita].[Data e Hora da Visita] AS DATE) = CAST(GETDATE() AS DATE) 
	ORDER BY 
		[Visita].[Número] DESC
		
	INSERT INTO [Histórico de Operações] (Sessão, Operação, Historico, [Registros Afetados]) 
	VALUES (@Sessao, 'Busca de Visitas do Dia', '', @@ROWCOUNT) 	
...
GO
GRANT EXECUTE
    ON OBJECT::[Representante].[Busca de Visitas do Dia] TO [altanet]
    AS [dbo];
