...
CREATE PROCEDURE [Representante].[Busca de Visitas da Semana Passada]
...
BEGIN
	...
	WHERE 
		...
		AND DATEPART(WEEK, [Visita].[Data e Hora da Visita]) = DATEPART(WEEK, GETDATE()) - 1
	ORDER BY 
		[Visita].[Número] DESC
...
GO
GRANT EXECUTE
    ON OBJECT::[Representante].[Busca de Visitas da Semana Passada] TO [altanet]
    AS [dbo];
