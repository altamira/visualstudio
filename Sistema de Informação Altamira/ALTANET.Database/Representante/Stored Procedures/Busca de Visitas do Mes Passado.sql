...
CREATE PROCEDURE [Representante].[Busca de Visitas do Mes Passado]
...
BEGIN
	...
	WHERE 
		...
		AND DATEPART(MONTH, [Visita].[Data e Hora da Visita]) = DATEPART(MONTH, GETDATE()) - 1
	ORDER BY 
		[Visita].[Número] DESC
...
GO
GRANT EXECUTE
    ON OBJECT::[Representante].[Busca de Visitas do Mes Passado] TO [altanet]
    AS [dbo];
